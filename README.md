...
Workflow
User enters the topic in the web interface.
User selects genre, mood, and scene length.
React frontend sends API request to FastAPI backend.
Backend generates embeddings for the input query.
FAISS vector database retrieves relevant screenplay knowledge.
Prompt engineering module constructs a structured prompt.
The prompt is sent to the Groq LLM API.
The LLM generates the screenplay scene.
The generated output is displayed to the user.
Technologies Used
Technology	Purpose
